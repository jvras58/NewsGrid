from app2.api.analyze.schemas import (
    AnalyzeRequest,
    AnalyzeResponse,
    MyReportsResponse,
    ReportResponse,
...
async def get_report_logic(task_id: str, user_id: int) -> ReportResponse:
    use_case = container.get_report_use_case()
    try:
        entity = await use_case.execute(task_id, user_id)
...
async def list_my_reports_logic(user_id: int) -> MyReportsResponse:
    use_case = container.list_my_reports_use_case()
    try:
        task_ids, _ = await use_case.execute(user_id, None, 1, 10)
